...
object Day10 {
  ...
  def solveB(instructions: List[Instruction]): String = {
    ...
    def render(instructionsLeft: List[Instruction], cycle: Int = 1, register: Int = 1, renderIdx: Int = 0): Unit = {
      ...
      val spritePosition: Seq[Int] = (register  - 1 to register + 1).map(_ % cols)
      if (spritePosition.contains(renderIdx % cols)) {
        crt(renderIdx / cols)(renderIdx % cols) = '#'
      }
